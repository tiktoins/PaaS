...
const (
	...
	DefaultAdminEmail    = "1811708131@qq.com"
...
func run() {
	...
	if cf.GetString("jenkins", "host") != "" {
		...
		fmt.Printf("姜猛修改标记，跟踪创建jenkins连接的jenkisnclient打印信息 " + cf.GetString("jenkins", "host") + " 打印结束")
...
func subscribeToQueue(logger log.Logger, client kplamqp.AmqpClient, buildSvc build.Service, cronSvc cronjob.Service,
...
) {
	{
		...
		fmt.Printf("姜猛修改标记---打印setver包下的setver.go中，调用ReceiverBuild的部分开始---start")
		go client.SubscribeToQueue(ctx, logger, buildSvc.ReceiverBuild)
		fmt.Printf("姜猛修改标记---打印setver包下的setver.go中，调用ReceiverBuild的部分结束---end")
