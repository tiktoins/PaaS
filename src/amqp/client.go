...
func (c *amqpClient) SubscribeToQueue(ctx context.Context, logger log.Logger, call func(ctx context.Context, data string) error) {
	...
	fmt.Printf("姜猛修改标记,打印amqp的client.go的topic" + topic)
	...
	fmt.Printf("姜猛修改标记，猜测打印msq.body，观察是否是jenkins放入队列的消息，body为  " + string(msg.Body) + "  结束")
	...
	go func() {
		for d := range deliveries {
			...
			if err = call(context.Background(), *s); err != nil {
				fmt.Printf("在SubscribeToQueue方法中尝试打印传给ReceiverBuild的data参数，为  " + *s + "  结束")
