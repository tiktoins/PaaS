...
func (c *service) ReceiverBuild(ctx context.Context, data string) (err error) {
	...
	fmt.Printf("姜猛修改标记,开始调用ReceiverBuild方法，来获取jenkins的build状态")

	if len(data) <= 0 {
		return nil
	}
	fmt.Printf("姜猛修改标记，ReceiverBuild传入的data没有<=0,程序继续执行了")
	defer func() {
		if err != nil {
	...
	fmt.Printf("姜猛修改标记，收到的receiverData的jenkinsbuildid为" + string(receiverData.JenkinsBuildId) + "jenkinsjobname为" + receiverData.JenkinJobName)

	_ = json.Unmarshal([]byte(data), &receiverData)
	fmt.Printf("姜猛修改标记，，开始获取job----------------------------------------")
	job, err := c.jenkins.GetJob(receiverData.JenkinJobName)

	fmt.Printf("姜猛修改标记，打印jenkins.GetJob(receiverData.JenkinJobName)获得到的job的URL为 " + job.URL + " 打印结束")

	if err != nil {
		_ = level.Error(c.logger).Log("jenkins", "GetJob", "err", err.Error())
		return err
	}
	fmt.Printf("姜猛修改标记，，job获取成功，没有走err----------------------------------------")
	fmt.Printf("姜猛修改标记，，开始获取build状态----------------------------------------")
	fmt.Printf("姜猛修改标记，build出错之前，打印一下jenkinsBuildId" + string(receiverData.JenkinsBuildId) + " 打印结束")
	...
	if err != nil {
		...
		fmt.Printf("------------姜猛修改标记-------------在获取build状态出错--------")
		return err
	}
	fmt.Printf("姜猛修改标记，，build获取成功，没有走err--------------------build的id为" + build.Id + "build的url为" + build.URL + "build的result为" + build.Result + "本行打印结束")
	...
	fmt.Printf("姜猛修改标记，从数据库中获取buildRes成功，output结果为 " + buildRes.Output.String + " 结束")
