...
func decodeBuildConsoleRequest(_ context.Context, r *http.Request) (interface{}, error) {
	fmt.Printf("姜猛修改标记decodeBuildConsoleRequest的地址rul的host为" + r.Host + "host结束" + "RemoteAddr为" + r.RemoteAddr + "path结束" + "RequestURI为" + r.RequestURI + "结束")
	fmt.Printf("姜猛打印标记，url的Proto打印出来是" + r.Proto + "结束")
	vars := mux.Vars(r)
	number, ok := vars["number"]
	if !ok {
		return nil, encode.ErrBadRoute
