...
func (c *service) AccessAudit(ctx context.Context, ns, name string) error {
	fmt.Printf("姜猛修改标记，开始调用”开始部署按钮对应的方法了-----------------------“")
	...
	fmt.Printf("姜猛修改标记，打印开始部署AccessAudit方法中的shell指令为：" + projectJenkins.Command)
	if err != nil {
	...
	if project.Language != repository.Java.String() {
		...
		fmt.Printf("姜猛修改标记，打印调用CreatJavaJobParams的地方的jenkins的shell指令为" + jenkinsParams.Command + "@@@@@@@@%%%%%+++++++")
